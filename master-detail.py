...
def display_table_columns(connection, table_name):
    ...
    try:
        ...
        if not columns:
            print(f"[ERROR] Table '{table_name}' not found in current database.")
            return

        print(f"\n[TABLE] Columns of table: `{table_name}`")
        print("-" * 80)
        ...
        print(f"[ERROR] Error fetching columns: {e}")
    finally:
        cursor.close()
...
def main():
    ...
    connection.close()
    print("\n[SUCCESS] Master-Detail CRUD generated successfully!")
